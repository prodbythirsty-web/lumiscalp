...
const SERUM_PRICE     = 9.99; // 45-day serum included in bundle 2
...
function calcTotal(items, fallbackCents) {
  if (items && items.length) {
    ...
    items.forEach(item => {
      ...
      total += ledPrice; // always 1 cap
      // Serum: bundle 2 only, and only if customer hasn't removed it (serum !== false)
      if (item.bundle === 2 && item.serum !== false) total += SERUM_PRICE;
