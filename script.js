...
function renderCart() {
  ...
  cartItems.forEach((item, idx) => {
    ...
    const serumRow = item.bundle === 2
      ? `<div class="cart-serum-row">
           <span class="cart-serum-label">${item.serum !== false ? '✓ Copper Hair Growth Serum (+$${SERUM_PRICE.toFixed(2)})' : '✗ Serum removed'}</span>
           <button class="cart-serum-toggle" onclick="toggleSerum(${idx})">${item.serum !== false ? 'Remove' : 'Add back'}</button>
         </div>`
      : '';
      ...
      <div class="cart-item">
        <div class="cart-item-top">
          ...
          <div class="cart-item-info">
            ...
            <div class="cart-item-detail">Red Light Therapy${item.bundle === 2 && item.serum !== false ? ' + Copper Serum' : ''}${item.guarantee ? ' + 90 Day Guarantee' : ''}</div>
...
function toggleSerum(idx) {
  cartItems[idx].serum = !cartItems[idx].serum;
...
function addToCart() {
  cartItems.push({
    ...
    serum:     selBundle === 2,   // serum included by default for bundle 2; removable in cart
  });
  updateBadge();
  renderCart();
