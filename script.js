...
function renderCart() {
  ...
  cartItems.forEach((item, idx) => {
    ...
    const hasMassager = item.massager !== false;
      ...
      <div class="cart-item">
        ...
        <div class="cart-item-bottom">
          <div class="qty-stepper">
            <button class="qty-btn" onclick="adjustCapQty(${idx}, -1)" aria-label="Decrease">−</button>
            <span class="qty-num">1</span>
            <button class="qty-btn" onclick="adjustCapQty(${idx}, 1)" aria-label="Increase">+</button>
          </div>
          <span class="cart-item-price-sm">$${capPrice.toFixed(2)}</span>
        </div>
      </div>`;

    totalDiscount += (capWas - capPrice);
      ...
      html += `
        <div class="cart-item cart-addon-row">
          <div class="cart-item-top">
            ...
            <button class="cart-item-trash" onclick="removeMassager(${idx})" aria-label="Remove">${TRASH_ICON}</button>
          </div>
          <div class="cart-item-bottom">
            <div class="qty-stepper">
              <button class="qty-btn" onclick="removeMassager(${idx})" aria-label="Remove">−</button>
              <span class="qty-num">1</span>
              <button class="qty-btn" disabled aria-label="Increase">+</button>
            </div>
      ...
      html += `
        <div class="cart-item cart-addon-row">
          <div class="cart-item-top">
            ...
            <button class="cart-item-trash" onclick="toggleSerum(${idx})" aria-label="Remove">${TRASH_ICON}</button>
          </div>
          <div class="cart-item-bottom">
            <div class="qty-stepper">
              <button class="qty-btn" onclick="toggleSerum(${idx})">−</button>
              <span class="qty-num">1</span>
              <button class="qty-btn" disabled>+</button>
            </div>
            <span class="cart-item-price-sm">$${SERUM_PRICE.toFixed(2)}</span>
          </div>
        </div>`;
      ...
      html += `
        <div class="cart-item cart-addon-row">
          <div class="cart-item-top">
            ...
            <button class="cart-item-trash" onclick="removeGuarantee(${idx})" aria-label="Remove">${TRASH_ICON}</button>
          </div>
          <div class="cart-item-bottom">
            <div class="qty-stepper">
              <button class="qty-btn" onclick="removeGuarantee(${idx})">−</button>
              <span class="qty-num">1</span>
              <button class="qty-btn" disabled>+</button>
            </div>
            <span class="cart-item-price-sm">$${GUARANTEE_PRICE.toFixed(2)}</span>
          </div>
        </div>`;
    }
  });
  ...
  if (countEl) {
    const totalUnits = cartItems.reduce((s, item) => {
      let n = 1; // cap
      if (item.massager !== false) n++;
      if (item.bundle === 2 && item.serum !== false) n++;
      if (item.guarantee) n++;
      return s + n;
    }, 0);
...
function adjustCapQty(idx, delta) {
  if (delta < 0) { removeItem(idx); }
  // qty is capped at 1 per item object; to add more, user adds to cart again
}
